...
class OSTrack(BaseTracker):
    def __init__(self, params, dataset_name):
        ...
        vocab_path = '/home/cv/wu/gitnew/JointNLT/pretrained/bert/bert-base-uncased-vocab.txt'
    ...
    def initialize(self, image, info: dict):
        self.oritext = '[CLS] '+info['init_nlp']+' [SEP]'
        self.text_input = self._text_input_process(info['init_nlp'], 40)
        #with torch.no_grad():
    ...
    def track(self, image, info: dict = None):
        ...
        with torch.no_grad():
            ...
            out_dict = self.network.forward(
                template=self.z_dict1.tensors, search=x_dict.tensors, ce_template_mask=self.box_mask_z,text_data=self.text_input)
        ...
        if self.debug:
            if not self.use_visdom:
                ...
                if 'removed_indexes_lq' in out_dict and out_dict['removed_indexes_lq']:
                    removed_indexes_lq= torch.cat(out_dict['removed_indexes_lq'], dim=1).cpu().tolist()[0]
                  
                    oritext_s = self.oritext.split()

                    for removed_index_lq in removed_indexes_lq:
                        index = int(removed_index_lq)
                        oritext_s[index]='[Mask]'
                    replacetext = ' '.join(oritext_s)

                    restext = self.oritext + '\n' + replacetext

                    self.visdom.register(restext, 'text', 1, 'Text1')
